<x-app-layout class="container mx-auto flex flex-col md:flex-row md:space-x-8 px-4">
    <div class="container mx-auto max-w-lg p-8 bg-white shadow-md rounded-lg">
        <h1 class="text-3xl text-center font-bold text-primary">Edit User</h1>

        <div class="relative w-full">
            <form id="userForm" action="{{ route('users.update', $user) }}" method="POST" class="p-4">
                @csrf
                @method('PUT')
                ...
                <div>
                    <label class="mt-4 block text-md font-medium text-primary">Password (opsional)</label>
                    <input type="password" name="password" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary" placeholder="Kosongkan jika tidak ingin mengubah password">
                </div>
