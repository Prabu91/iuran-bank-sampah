<x-app-layout class="container mx-auto flex flex-col md:flex-row md:space-x-8 px-4">
    <div class="container mx-auto max-w-lg p-8 bg-white shadow-md rounded-lg">
        <h1 class="text-3xl text-center font-bold text-blue-600">Edit Data Unit</h1>

        <div class="relative w-full">
            <form id="unitForm" action="{{ route('unitbsb.update', $unitbsb->id) }}" method="POST" class="p-4">
                @csrf
                @method('PUT')

                <div>
                    <label class="mt-4 block text-md font-medium text-gray-700">Nama Unit</label>
                    <input type="text" name="unit_name" value="{{ old('unit_name', $unitbsb->unit_name) }}"
                ...
                <div>
                    ...
                    <input type="text" name="pic_name" value="{{ old('pic_name', $unitbsb->pic_name) }}"
                ...
                <div>
                    ...
                    <textarea name="address"
                        ...
                        placeholder="Alamat" required>{{ old('address', $unitbsb->address) }}</textarea>
                    @error('address')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="phone" value="{{ old('phone', $unitbsb->phone) }}"
            ...
            <div id="confirmationModal" class="fixed inset-0 z-50 hidden bg-gray-800 bg-opacity-75 flex items-center justify-center">
                <div class="bg-white rounded-lg shadow-lg p-6 max-w-sm w-full">
                    ...
                    <p>Apakah Anda yakin ingin memperbarui data unit ini?</p>
